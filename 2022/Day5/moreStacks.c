...
void moveCrates(int cratesToMove, int srcStack, int destStack) {
	...
	for (int i = 0; i < cratesToMove; i++){
	    push(9 , stackOne[topOne]);
	    pop(srcStack - 1);
	}

	for (int i = 0; i < cratesToMove; i++) {
	    push(destStack - 1, tempStack[topTemp]);
	    pop(9);
	}
	break;
    case 2:
	for (int i = 0; i < cratesToMove; i++){
	    push(9 , stackTwo[topTwo]);
	    pop(srcStack - 1);
	}
